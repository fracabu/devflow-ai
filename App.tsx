...
${cleanContent}`;
  ...
  const handleSaveToPlan = () => {
    ...
    if (viewingSavedArticle) {
      ...
      // Create new item
      const currentModelName = AVAILABLE_MODELS[providerConfig.provider]?.find(m => m.id === (providerConfig.model || DEFAULT_MODELS[providerConfig.provider]))?.name || providerConfig.model || 'Unknown';
      const newItem: EditorialItem = {
        ...
        articleData: { ...article },
        generatedWith: currentModelName
      };
